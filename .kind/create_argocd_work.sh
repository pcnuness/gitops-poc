...
# Function to cleanup all clusters
cleanup_clusters() {
    ...
    echo "All clusters have been removed."
    exit 0
}
...
EOF
    ...
    # Patch the Nginx Ingress deployment to add the --enable-ssl-passthrough argument
    echo "Patching Nginx Ingress deployment to enable SSL passthrough..."
    kubectl patch deployment ingress-nginx-controller -n ingress-nginx --type='json' -p='[{"op": "add", "path": "/spec/template/spec/containers/0/args/-", "value": "--enable-ssl-passthrough"}]' --context kind-${cluster_name}
...
echo
echo "ArgoCD is accessible at https://argocd.local:${BASE_HTTPS_PORT}"
